...
nBehav = size(Y0, 2); % number of behavioral variables
nGroups = size(unique(grouping), 1); % number of groups

%% ========== Main bootstrap loop ========== %%
disp('... Bootstrapping ...')

% Compute bootstrap sampling order (whether within-group sampling is determined by grouped_boot)
...
for iB = 1:pls_opts.nBootstraps
    ...
    Xb = X0(all_boot_orders(:, iB), :); % resample according to bootstrap order
    Xb = myPLS_norm(Xb, grouping, pls_opts.normalization_img); % normalize
    ...
    %% Procrustes transform (correct axis rotation/reflection)
    switch pls_opts.boot_procrustes_mod
        case 1
            % Compute rotation only on U
...
% Save all statistical fields into boot_results for future extensibility
